...
VehicleCouting::VehicleCouting() : firstTime(true), showOutput(true), key(0), countAB(0), countBA(0), countCD(0), countDC(0), showAB(1)
{
...
VehiclePosition VehicleCouting::getVehiclePosition(const CvPoint2D64f centroid)
{
  ...
  {
	if(centroid.x < (FAV1::roi_x0 + FAV1::roi_x1)/2 && centroid.y > std::min(FAV1::roi_y0, FAV1::roi_y1) && centroid.y < std::max(FAV1::roi_y0, FAV1::roi_y1))
    {
	...
	if(centroid.x < (FAV1::roi_x2 + FAV1::roi_x3)/2 && centroid.y > std::min(FAV1::roi_y2, FAV1::roi_y3) && centroid.y < std::max(FAV1::roi_y2, FAV1::roi_y3))
    {
      cv::putText(img_input, "C", cv::Point(10,img_h/2), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255,255,255));
      vehiclePosition = VP_C;
    }
    
	if(centroid.x > (FAV1::roi_x2 + FAV1::roi_x3)/2 && centroid.y > std::min(FAV1::roi_y2, FAV1::roi_y3) && centroid.y < std::max(FAV1::roi_y2, FAV1::roi_y3))
    {
      cv::putText(img_input, "D", cv::Point(10,img_h/2), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255,255,255));
  ...
  if(LaneOrientation == LO_VERTICAL)
  {
	...
	if(centroid.y < (FAV1::roi_y2 + FAV1::roi_y3)/2 && centroid.x > std::min(FAV1::roi_x2, FAV1::roi_x3) && centroid.x < std::max(FAV1::roi_x2, FAV1::roi_x3))
    {
      cv::putText(img_input, "C", cv::Point(10,img_h/2), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255,255,255));
      vehiclePosition = VP_C;
    }
...
void VehicleCouting::process()
{
  ...
  if(FAV1::use_roi == true && FAV1::roi_defined == true)
  {
	...
	cv::line(img_input, cv::Point(FAV1::roi_x2,FAV1::roi_y2), cv::Point(FAV1::roi_x3,FAV1::roi_y3), cv::Scalar(0,255,0));
  }
  ...
  if(ROI_OK)
  {
    ...
    if(abs(FAV1::roi_x0 - FAV1::roi_x1) < abs(FAV1::roi_y0 - FAV1::roi_y1))
    {
	  ...
	  cv::putText(img_input, "(C)", cv::Point(FAV1::roi_x2-32,FAV1::roi_y2), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255,255,255));
      cv::putText(img_input, "(D)", cv::Point(FAV1::roi_x2+12,FAV1::roi_y2), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255,255,255));
    }
  ...
  if(showAB == 0)
  {
    cv::putText(img_input, countABstr, cv::Point(10, img_h - 44), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
    cv::putText(img_input, countBAstr, cv::Point(10, img_h - 32), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
	cv::putText(img_input, countCDstr, cv::Point(10, img_h - 20), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
	cv::putText(img_input, countDCstr, cv::Point(10, img_h - 8), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
  }
  
  if(showAB == 1)
  {
	cv::putText(img_input, countABstr, cv::Point(10, img_h - 20), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
	cv::putText(img_input, countCDstr, cv::Point(10, img_h - 8), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
  }

  if(showAB == 2)
  {
	cv::putText(img_input, countBAstr, cv::Point(10, img_h - 20), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
	cv::putText(img_input, countDCstr, cv::Point(10, img_h - 8), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
  }
...
void VehicleCouting::saveConfig()
{
  ...
  cvWriteInt(fs, "fav1_roi_x2", FAV1::roi_x2);
  cvWriteInt(fs, "fav1_roi_y2", FAV1::roi_y2);
  cvWriteInt(fs, "fav1_roi_x3", FAV1::roi_x3);
  cvWriteInt(fs, "fav1_roi_y3", FAV1::roi_y3);
...
void VehicleCouting::loadConfig()
{
  ...
  FAV1::roi_x2 = cvReadIntByName(fs, 0, "fav1_roi_x2", 0);
  FAV1::roi_y2 = cvReadIntByName(fs, 0, "fav1_roi_y2", 0);
  FAV1::roi_x3 = cvReadIntByName(fs, 0, "fav1_roi_x3", 0);
  FAV1::roi_y3 = cvReadIntByName(fs, 0, "fav1_roi_y3", 0);
